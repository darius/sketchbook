...
def parse(peg, s):
    ...
    for (value,), rest in as_peg(peg)(s): return value
...
def give(vals):
    return Peg(lambda s: [(a(tuple, vals), s)])
def take(peg, f):
    return Peg(lambda s: [res for vals, s1 in peg(s) for res in f(a(tuple, vals))(s1)])

def singleton(x): return (x,)

def cons(peg, rest_peg):
    return take(peg, lambda vals: take(rest_peg, lambda (rest,):
             give(singleton(list(a(tuple, vals)) + a(list, rest)))))
